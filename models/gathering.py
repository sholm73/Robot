...
class Gather():
    ...
    def getCurrencyDetails(self):
        r = requests.get(self.base_api + 'v1/symbols_details/')
        return r.json()
    ...
    def getFundBookInfo(self, currency):
        r = requests.get(self.base_api + 'v1/lendbook/' + currency + '/')
        return r.json()

    # Get the full order book. Return: price, amount, timestamp
    def getOrderBookInfo(self, pair):
        r = requests.get(self.base_api + 'v1/book/' + pair + '/')
        return  r.json()
    ...
    def getTradeInfo(self, pair):
        r = requests.get(self.base_api + 'v1/trades/' + pair + '/')
    ...
    def getLendInfo(self, pair):
        r = requests.get(self.base_api + 'v1/lends/' + pair + '/')
